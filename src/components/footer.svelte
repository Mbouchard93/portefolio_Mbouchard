<script>
  ...
  function handleContactClick() {
    window.location.href = 'mailto:mariiane.bouchard12@gmail.com'; 
  }
</script>

<footer class="text-blueLight font-heading container mx-auto px-6 lg:block flex flex-col items-center pt-4">
  <div class="flex justify-between py-4">
    <div class="flex flex-col gap-6">
      <h2>Mariane Bouchard</h2>
      <button 
        class="px-[2.25rem] py-[0.5rem] border shadow-btnContact border-blueDark text-[1.25rem] rounded-[0.3125rem] text-blueDark bg-blueLight hover:shadow-btnContactHover hover:translate-y-1" 
      ...
      <ul class="flex gap-10">
        {#each sections as section (section.id)}
          {#if section.id !== 'contact'}
            <li>
              <a class="hover:underline" href="#{section.id}">
                {section.name}
              </a>
            </li>
          {/if}
    ...
    <div class="hidden lg:block">
      ...
      <div>
        <p>Tél : 514-249-2378</p>
        <p>Mariiane.bouchard12@gmail.com</p>
        <p>Lanaudière, Québec</p>
      </div>
    </div>
  </div>
  <div class="flex flex-col lg:flex-row lg:justify-between items-center py-4 border-t border-t-accent w-full gap-4">
    <div class="flex gap-10 lg:order-2">
    ...
    <p>© Mariane Bouchard 2024 - Tous droits réservés</p>
  </div>
</footer>
